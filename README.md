...
- 任务窗格中列出所有已打开的 Excel 窗口，高亮当前活动窗口，点击切换（≤1.5s 内自动同步）。
- 对齐模式：开启时记录各窗口原始位置，并把所有窗口设为同一几何尺寸；关闭时恢复原位置。
...
- 标签颜色与分组、拖拽排序、MRU 排序、⌘1–9 快捷键。
- 悬浮对话框切换器（需先验证 dialog 是否可跨窗口常驻置顶）。
