...
4. 点击**对齐：关/开**切换对齐模式：开启后所有窗口叠放到相同位置与大小，切换时看起来像一个窗口换标签；关闭会尽量恢复各窗口原始位置。
   注意：macOS 版 Excel 的窗口几何写入**只对前台窗口生效**，因此对齐是在"窗口被激活时"套用的——点击标签、按 ⌘` 切换或新打开文档后，该窗口会在 1 秒内自动对齐；对齐模式下窗口被拖歪也会被自动拉回基准位置。
...
- 对齐模式下窗口几何写入只对前台窗口生效：后台窗口会在被激活时才对齐（属于 Excel 在 macOS 上的限制）。
